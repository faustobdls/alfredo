...
length:
  max_words: 220
format:
  target: markdown                # markdown | plain | email | pptx | marp |
                                  # gamma | figma-slides | docx | html
...
alfredo package install bank-templates --target claude-code
```
...
| `alfredo template new <name> --kind <kind>` | Scaffold `templates/<name>/TEMPLATE.md` |
...
1. `alfredo template new <name> --kind <kind>`.
2. Fill in `voice`, `structure`, `length`, `constraints`, and the body.
3. `alfredo template validate <name>`.
4. Ship it in a package's `contents.templates`, or keep it in the project.
